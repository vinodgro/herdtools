...
\subsubsection{Communication candidate relaxations}
...
\item[{\tt -mode (critical|sc|free)}]
Control generation  of cycles, default~\opt{sc}.
Critical mode offers predictive control on cycle generation (see below),
and is activated when \diy{} is invoked by the automated front-end~\dont.
\item[{\tt -cumul <bool>}]
...
Generally speaking, \diy{} generates ``some'' cycles and does not generate
...
As an exception to this non-specification, \diy{} is guaranteed not to
...
\begin{enumerate}
\item Communication candidate relaxations sequences are
limited to Rf,Fr,Ws,[Ws,Rf] and~[Fr,Rf], as in sc and critical mode.
\end{enumerate}
