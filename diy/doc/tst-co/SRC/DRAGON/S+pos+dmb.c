...
static void do_dump_outcome(FILE *fhist, int *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:R0=%i; x=%i;\n",c,show ? '*' : ':',o[out_1_r0_f],o[x_f]);
}
...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test S+pos+dmb Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+pos+dmb");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"S+pos+dmb: global x unstabilized") ;
      }
      cond = final_cond();
...
static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/S+pos+dmb.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM S+pos+dmb");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "S+pos+dmb: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","4aadc1b8c62acbae4e38893a221dcc8f");
  fprintf(out,"%s=%s\n","Cycle","Rfe DMBsRW Wse PosWW");
  fprintf(out,"Relax S+pos+dmb %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Rfe Wse PosWW DMBsRW");
  fprintf(out,"%s=%s\n","Com","Rf Ws");
  fprintf(out,"%s=%s\n","Orig","PosWW Rfe DMBsRW Wse");
  fprintf(out,"Observation S+pos+dmb %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time S+pos+dmb %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int S_2B_pos_2B_dmb(int argc, char **argv, FILE *out) {
