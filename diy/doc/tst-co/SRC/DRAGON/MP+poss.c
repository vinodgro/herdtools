...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test MP+poss Required\n") ;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"MP+poss: global x unstabilized") ;
      }
      cond = final_cond();
...
static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/MP+poss.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM MP+poss");
  fprintf(out,"%s\n","\"PosWW Rfe PosRR Fre\"");
  fprintf(out,"%s\n","{0:R2=x; 1:R2=x;}");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "MP+poss: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","3a31ee4acc87d7f6f4313dc5693886d1");
