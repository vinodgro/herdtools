...
static void do_dump_outcome(FILE *fhist, int *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:R1=%i; 1:R1=%i; x=%i;\n",c,show ? '*' : ':',o[out_0_r1_f],o[out_1_r1_f],o[x_f]);
}
...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test SB+dmb+pos Required\n") ;
...
static void init(ctx_t *_a) {
  ...
  _a->seed = rand();
  _a->out_0_r1 = malloc_check(size_of_test*sizeof(*(_a->out_0_r1)));
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+dmb+pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ldr %[r1],[%[r2]]\n"
"@END_litmus\n\t"
:[r1] "=&r" (out_0_r1[_i]),[r0] "=&r" (trashed_r0)
:[r2] "r" (&x[_i])
:"cc","memory"
);
    }
...
"ldr %[r1],[%[r2]]\n"
"@END_litmus\n\t"
:[r1] "=&r" (out_1_r1[_i]),[r0] "=&r" (trashed_r0)
:[r2] "r" (&x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      int _out_0_r1_i = ctx.out_0_r1[_i];
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"SB+dmb+pos: global x unstabilized") ;
      }
      cond = final_cond();
      o[out_0_r1_f] = _out_0_r1_i;
...
#else
#include "SB+dmb+pos.h"
#endif

static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/SB+dmb+pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM SB+dmb+pos");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "SB+dmb+pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","f3a0a66450643475b9155ca92a10d803");
  fprintf(out,"%s=%s\n","Cycle","Fre PosWR Fre DMBsWR");
  fprintf(out,"Relax SB+dmb+pos %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Fre PosWR DMBsWR");
  fprintf(out,"%s=%s\n","Com","Fr Fr");
  fprintf(out,"%s=%s\n","Orig","DMBsWR Fre PosWR Fre");
  fprintf(out,"Observation SB+dmb+pos %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time SB+dmb+pos %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int SB_2B_dmb_2B_pos(int argc, char **argv, FILE *out) {
