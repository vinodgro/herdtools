...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test S+poss Required\n") ;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"S+poss: global x unstabilized") ;
      }
      cond = final_cond();
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "S+poss: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","7c6d601d8f807d5ee2e484295297dfc2");
