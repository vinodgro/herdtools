...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test MP+pos+dmb Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+pos+dmb");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"MP+pos+dmb: global x unstabilized") ;
      }
      cond = final_cond();
...
#else
#include "MP+pos+dmb.h"
#endif

static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/MP+pos+dmb.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM MP+pos+dmb");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "MP+pos+dmb: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","52a68c4bc8752ea876ad9407f4e75231");
  fprintf(out,"%s=%s\n","Cycle","Rfe DMBsRR Fre PosWW");
  fprintf(out,"Relax MP+pos+dmb %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Rfe Fre PosWW DMBsRR");
  fprintf(out,"%s=%s\n","Com","Rf Fr");
  fprintf(out,"%s=%s\n","Orig","PosWW Rfe DMBsRR Fre");
  fprintf(out,"Observation MP+pos+dmb %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time MP+pos+dmb %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int MP_2B_pos_2B_dmb(int argc, char **argv, FILE *out) {
