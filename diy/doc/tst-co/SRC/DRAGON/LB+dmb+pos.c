...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test LB+dmb+pos Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+dmb+pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"LB+dmb+pos: global x unstabilized") ;
      }
      cond = final_cond();
...
#else
#include "LB+dmb+pos.h"
#endif

static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/LB+dmb+pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM LB+dmb+pos");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "LB+dmb+pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","1950023ac1c9aa9d9fbc7e6fa52902de");
  fprintf(out,"%s=%s\n","Cycle","Rfe PosRW Rfe DMBsRW");
  fprintf(out,"Relax LB+dmb+pos %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Rfe PosRW DMBsRW");
  fprintf(out,"%s=%s\n","Com","Rf Rf");
  fprintf(out,"%s=%s\n","Orig","DMBsRW Rfe PosRW Rfe");
  fprintf(out,"Observation LB+dmb+pos %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time LB+dmb+pos %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int LB_2B_dmb_2B_pos(int argc, char **argv, FILE *out) {
