...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test WW Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"WW: global x unstabilized") ;
      }
      cond = final_cond();
      o[x_f] = _x_i;
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "WW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","f575fac9103fb7ca0f70be4d495d4b52");
  fprintf(out,"%s=%s\n","Cycle","Wse PosWW");
  fprintf(out,"Relax WW %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Wse PosWW");
  fprintf(out,"%s=%s\n","Com","Ws");
  fprintf(out,"%s=%s\n","Orig","PosWW Wse");
  fprintf(out,"Observation WW %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time WW %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int WW(int argc, char **argv, FILE *out) {
