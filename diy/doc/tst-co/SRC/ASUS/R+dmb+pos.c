...
static void do_dump_outcome(FILE *fhist, int *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:R1=%i; x=%i;\n",c,show ? '*' : ':',o[out_1_r1_f],o[x_f]);
}
...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test R+dmb+pos Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"R+dmb+pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"str %[r1],[%[r2]]\n"
"@END_litmus\n\t"
:[r1] "=&r" (trashed_r1),[r0] "=&r" (trashed_r0)
:[r2] "r" (&x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"R+dmb+pos: global x unstabilized") ;
      }
      cond = final_cond();
...
static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/R+dmb+pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM R+dmb+pos");
  fprintf(out,"%s\n","\"DMBsWW Wse PosWR Fre\"");
  fprintf(out,"%s\n","{0:R2=x; 1:R2=x;}");
  fprintf(out,"%s\n"," P0          | P1          ;");
  fprintf(out,"%s\n"," MOV R0,#1   | MOV R0,#3   ;");
  fprintf(out,"%s\n"," STR R0,[R2] | STR R0,[R2] ;");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "R+dmb+pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","04f033c19b3f3bb665ec3439210f581f");
  fprintf(out,"%s=%s\n","Cycle","Fre DMBsWW Wse PosWR");
  fprintf(out,"Relax R+dmb+pos %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Fre Wse PosWR DMBsWW");
  fprintf(out,"%s=%s\n","Com","Ws Fr");
  fprintf(out,"%s=%s\n","Orig","DMBsWW Wse PosWR Fre");
  fprintf(out,"Observation R+dmb+pos %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time R+dmb+pos %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int R_2B_dmb_2B_pos(int argc, char **argv, FILE *out) {
