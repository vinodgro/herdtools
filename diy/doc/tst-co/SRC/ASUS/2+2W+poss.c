...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test 2+2W+poss Required\n") ;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"2+2W+poss: global x unstabilized") ;
      }
      cond = final_cond();
      o[x_f] = _x_i;
...
static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/2+2W+poss.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM 2+2W+poss");
  fprintf(out,"%s\n","\"PosWW Wse PosWW Wse\"");
  fprintf(out,"%s\n","{0:R2=x; 1:R2=x;}");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "2+2W+poss: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","e824c39a8731b08b8978dbb6f6b76b42");
...
int _2_2B_2W_2B_poss(int argc, char **argv, FILE *out) {
