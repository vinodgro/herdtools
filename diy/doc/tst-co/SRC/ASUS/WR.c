...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test WR Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WR");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"WR: global x unstabilized") ;
      }
      cond = final_cond();
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "WR: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","d1a3793941f60778d72cf4610125b2d9");
  fprintf(out,"%s=%s\n","Cycle","Fre PosWR");
  fprintf(out,"Relax WR %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Fre PosWR");
  fprintf(out,"%s=%s\n","Com","Fr");
  fprintf(out,"%s=%s\n","Orig","PosWR Fre");
  fprintf(out,"Observation WR %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time WR %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int WR(int argc, char **argv, FILE *out) {
