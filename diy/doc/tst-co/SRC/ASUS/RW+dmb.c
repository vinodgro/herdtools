...
static void do_dump_outcome(FILE *fhist, int *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:R0=%i; x=%i;\n",c,show ? '*' : ':',o[out_0_r0_f],o[x_f]);
}
...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test RW+dmb Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RW+dmb");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"RW+dmb: global x unstabilized") ;
      }
      cond = final_cond();
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "RW+dmb: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","a84af5a900d9197c7edcb4e472add8e5");
  fprintf(out,"%s=%s\n","Cycle","Rfe DMBsRW");
  fprintf(out,"Relax RW+dmb %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Rfe DMBsRW");
  fprintf(out,"%s=%s\n","Com","Rf");
  fprintf(out,"%s=%s\n","Orig","DMBsRW Rfe");
  fprintf(out,"Observation RW+dmb %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time RW+dmb %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int RW_2B_dmb(int argc, char **argv, FILE *out) {
