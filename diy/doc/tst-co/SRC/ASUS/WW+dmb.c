...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test WW+dmb Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WW+dmb");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"WW+dmb: global x unstabilized") ;
      }
      cond = final_cond();
      o[x_f] = _x_i;
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "WW+dmb: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","0a9f250ba8b4ce76ae09259e322513f7");
  fprintf(out,"%s=%s\n","Cycle","Wse DMBsWW");
  fprintf(out,"Relax WW+dmb %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Wse DMBsWW");
  fprintf(out,"%s=%s\n","Com","Ws");
  fprintf(out,"%s=%s\n","Orig","DMBsWW Wse");
  fprintf(out,"Observation WW+dmb %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time WW+dmb %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int WW_2B_dmb(int argc, char **argv, FILE *out) {
