...
static void do_dump_outcome(FILE *fhist, int *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:R0=%i; 1:R1=%i; x=%i;\n",c,show ? '*' : ':',o[out_1_r0_f],o[out_1_r1_f],o[x_f]);
}
...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test MP+dmb+pos Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+dmb+pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ldr %[r1],[%[r2]]\n"
"@END_litmus\n\t"
:[r0] "=&r" (out_1_r0[_i]),[r1] "=&r" (out_1_r1[_i])
:[r2] "r" (&x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"MP+dmb+pos: global x unstabilized") ;
      }
      cond = final_cond();
...
#else
#include "MP+dmb+pos.h"
#endif

static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/MP+dmb+pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM MP+dmb+pos");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "MP+dmb+pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","adf77cf97f7bd9bb8d4d24915fa2313c");
  fprintf(out,"%s=%s\n","Cycle","Rfe PosRR Fre DMBsWW");
  fprintf(out,"Relax MP+dmb+pos %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Rfe Fre PosRR DMBsWW");
  fprintf(out,"%s=%s\n","Com","Rf Fr");
  fprintf(out,"%s=%s\n","Orig","DMBsWW Rfe PosRR Fre");
  fprintf(out,"Observation MP+dmb+pos %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time MP+dmb+pos %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int MP_2B_dmb_2B_pos(int argc, char **argv, FILE *out) {
