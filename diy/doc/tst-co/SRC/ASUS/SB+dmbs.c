...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test SB+dmbs Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+dmbs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"SB+dmbs: global x unstabilized") ;
      }
      cond = final_cond();
...
static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/SB+dmbs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM SB+dmbs");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "SB+dmbs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","84c974f8e12c69ba6ba0c48da8d6d6a8");
  fprintf(out,"%s=%s\n","Cycle","Fre DMBsWR Fre DMBsWR");
  fprintf(out,"Relax SB+dmbs %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Fre DMBsWR");
  fprintf(out,"%s=%s\n","Com","Fr Fr");
  fprintf(out,"%s=%s\n","Orig","DMBsWR Fre DMBsWR Fre");
  fprintf(out,"Observation SB+dmbs %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time SB+dmbs %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int SB_2B_dmbs(int argc, char **argv, FILE *out) {
