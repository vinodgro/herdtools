...
static void dump_hist(FILE *fhist,hist_t *h) {
  fprintf(fhist,"Test S+dmb+pos Required\n") ;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+dmb+pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) err(1,"S+dmb+pos: global x unstabilized") ;
      }
      cond = final_cond();
...
static void report(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for src/S+dmb+pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","ARM S+dmb+pos");
...
/* Show parameters to user */
  if (prm.verbose) {
    fprintf(stderr, "S+dmb+pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  free_hist(hist);
  fprintf(out,"%s=%s\n","Hash","6a170644019c2ab6c6b970b7baa62cfc");
  fprintf(out,"%s=%s\n","Cycle","Rfe PosRW Wse DMBsWW");
  fprintf(out,"Relax S+dmb+pos %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"%s=%s\n","Safe","Rfe Wse PosRW DMBsWW");
  fprintf(out,"%s=%s\n","Com","Rf Ws");
  fprintf(out,"%s=%s\n","Orig","DMBsWW Rfe PosRW Wse");
  fprintf(out,"Observation S+dmb+pos %s %"PCTR" %"PCTR"\n",!p_true ? "Never" : !p_false ? "Always" : "Sometimes",p_true,p_false) ;
  fprintf(out,"Time S+dmb+pos %.2f\n",total / 1000000.0) ;
  fflush(out);
  return(p_true && p_false);
}

int S_2B_dmb_2B_pos(int argc, char **argv, FILE *out) {
