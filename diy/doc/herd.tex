...
in Section~\ref{language:instruction}
for the syntax and semantics of bindings.
...
\alt \T{fun} \NT{fundef} \brepet{} \T{and} \NT{fundef} \erepet{}
...
\sep
\NT{fundef} \is \NT{id} \T{(} \NT{params} \T{)} \T{=} \NT{expr}
\sep
...
\subsubsection*{\label{bindings}Bindings}
...
The \T{fun} construct defines functions for the rest
of model execution:
\begin{center}
\synt{\T{fun} \NT{id}_1 \T{(} \NT{params}_1 \T{)} \T{=} \NT{expr}_1 \T{and} \ldots \T{and} \NT{id}_n \T{(} \NT{params}_n \T{)} \T{=} \NT{expr}_n}
\end{center}
defines $n$ functions \synt{\NT{id}_1},\ldots , \synt{\NT{id}_n}.
Function arity is defined by the length of its formal parameter list
\NT{params}, which can be empty.
Notice that functions have the usual static scoping semantics:
variables that appear free in function bodies 
(\synt{\NT{expr}_1,\ldots,\NT{expr}_n} above) are bound the
