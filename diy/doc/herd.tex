...
The structure of model files has been updated to include a {\tt withco} flag, which is given directly after the name of the model. The flag identifies whether this model will constrain the coherence order (\emph{co}) relation, and hence whether that relation needs to be generated in executions. (\herd{} can execute faster in the absence of this relation.) 
...
Models can now define expressions over sets, and also bind identifiers to sets. Currently, functions only consume and produce relations, not sets. The keyword \synt{\T{rln}} is given when defining a relation, and the keyword \synt{\T{set}} is given when defining a set. Relations remain the default entities, so \synt{\T{rln}} can be omitted. The \synt{\NT{valbinding}} and \synt{\NT{funbinding}} syntactic categories thus become:

\begin{syntax}
\NT{funbinding} \is \boption \T{rln} \eoption \NT{id} \T{(} \NT{params} \T{)} \T{=} \NT{expr}
\sep
\NT{valbinding} \is \boption \T{rln} \eoption \NT{id} \T{=} \NT{expr}
\alt
\T{set} \NT{id} \T{=} \NT{set-expr}
\end{syntax}

The following expressions over sets are supported.

\begin{syntax}
\NT{set-expr} \is{} \T{0}
\alt \T{\_}
\alt \NT{id}
\alt \T{\textasciitilde}\NT{set-expr} 
\alt \NT{set-expr}\T{|}\NT{set-expr} \orelse
\NT{set-expr}\T{\textbackslash}\NT{set-expr} \orelse
\NT{set-expr}\T{\&}\NT{set-expr}
\alt \T{(}\NT{set-expr}\T{)}
\end{syntax}

We write \synt{\T{0}} for the empty set, \synt{\T{\_}} for the set of all events in the event structure, \synt{\T{|}} for union, \synt{\T{\&}}  for intersection, \synt{\T{\textbackslash}}  for difference, and \synt{\T{\textasciitilde}} for complement.

We provide two new constructs for building relations. If $s$ and $t$ are terms of type \synt{\NT{set-expr}}, then \synt{\T{[} \text{$s$} \T{*} \text{$t$} \T{]}} builds their cross product. We also provide \synt{\T{[} \text{$s$} \T{]}}, which denotes the relation that links each element of $s$ to itself. The syntax of expressions thus becomes:

\begin{syntax}
\NT{expr} \is{} \dots 
\alt \T{[} \NT{set-expr} \T{*} \NT{set-expr} \T{]}
\alt \T{[} \NT{set-expr} \T{]}
\end{syntax}
...
{\tt rmw} & blocked read-modify-writes & the set of all events representing a blocked read-modify-write \\
...
{\tt acq_rel} & acquire/release & atomic events with ``acquire/release'' memory-order \\
