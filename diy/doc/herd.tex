...
On sees that the test condition does not validate on top of SC,
as the ``\texttt{No}'' appears just after the list of final states
...
two absurd behaviours prictured in the execution diagram above:
...
For any model, we rule out those very untimely behaviours by the so-called
\textsc{uniproc}
...
However, users can reproduce the style of teh diagram of this manual using
...
Infix operators are \T{|} (union), \T{;} (sequence) and~\T{\&} (intersection).
The precedence of operators is as follows:
postfix operators bind tighter than~\T{;} and~\T{\&},
which bind tighter than~\T{|}. All infix operators are right-associative,
which their semantics will not reveal anyway.
...
\alt \T{show} \brepet \NT{id} \erepet
\alt \T{unshow} \brepet \NT{id} \erepet
\sep
\NT{binding} \is \NT{id} \T{=} \NT{expr}
\sep
...
of model execution. The construct
\begin{center}
\synt{\T{let} \NT{id}_1 \T{=} \NT{expr}_1 \T{and} \ldots \T{and} \NT{id}_n \T{=} \NT{expr}_n}
\end{center}
evaluates \synt{\nt{expr}_1,\ldots, \nt{expr}_n},
...
\begin{center}
\synt{\T{let} \T{rec} \NT{id}_1 \T{=} \NT{expr}_1 \T{and} \ldots \T{and} \NT{id}_n \T{=} \NT{expr}_n}
\end{center}
...
\end{verbatim}
Notice that the instruction \verb-let complus = (rf|co|rf)+- is equivalent.
...
thanks to the \opt{-skipcheck~}\nt{id} command line option.


\subsubsection*{Show (and unshow) directives}
\label{show:def}The constructs:
\begin{center}
\synt{\T{show} \brepet \NT{id} \erepet}\quad{and}\quad\synt{\T{unshow} \brepet \NT{id} \erepet}
\end{center}
take lists of identifiers as arguments.
...
This option applies to text file models. It instructs \herd{} to ignore
...
text file models. Notice that \opt{-model uniproc}
and \opt{-model minimal -optpace true} should yield identical results,
teh second being faster.
...
\verb+show+ directive in model definition.
\end{description}
