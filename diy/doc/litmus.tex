...
Supplying the tags
\opt{custom},\opt{static} or~\opt{static2} to \prog{litmus} command line
option \opt{-preload} commands the insertion of cache prefetch or flush
instructions before every test instance.

In custom mode the execution of such cache management instruction
is under total user control, the other two, ``static'' modes offer
...
% ./6.SB.exe -pra T
\end{verbatim}
will perform a run where every test thread touches all test locations
(\emph{i.e.} \texttt{x}, \texttt{y}, \texttt{z}, \texttt{a}, \texttt{b},
\texttt{c}) before loop synchronising and executing test code proper.
...
Test 6.SB Allowed
Histogram (63 states)
63    *>0:EAX=0; 1:EAX=0; 2:EAX=0; 3:EAX=0; 4:EAX=0; 5:EAX=0;
...
Witnesses
Positive: 63, Negative: 999937
...
...
When prefetch metadata is available it sets the
default of prefetch directives:
\begin{verbatim}
...
Test 6.SB Allowed
Histogram (63 states)
500   *>0:EAX=0; 1:EAX=0; 2:EAX=0; 3:EAX=0; 4:EAX=0; 5:EAX=0;
...
Prefetch=0:y=T,0:x=F,1:z=T,1:y=F,2:z=F,2:a=T,3:b=T,3:a=F,4:c=T,4:b=F,5:x=T,5:c=F
...
\end{verbatim}
...
See the next section for the syntax of configuration files.
\item[{\tt -o <dest>}]
...
\item[{\tt -crossrun <(user@)?host(:port)?|adb>}] \aname{crossrun}{Instruct}
the \texttt{run.sh} script to run
individual tests on a remote machine.
...
Default is \opt{self}.
\item[{\tt -prealloc <bool>}]
Enable or disable pre-allocation mode, default disabled.
In pre-allocation mode, memory is allocated before forking any thread.
\item[{\tt -preload (no|random|custom|static)}]
Specify preload mode (see Sec.~\ref{defpreload}), default is \texttt{random}.
Starting from version 5.0 we provide the additional \texttt{custom}
and \texttt{static} preload mode,
for a finer control of pre-fetching and flushing of some memory locations
by some threads. The feature is experimental and undocumented.
...
The following option is significant when affinity control is enabled.
...
The new items are defined in mapping files whose names are arguments to
