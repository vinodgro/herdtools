...
with type v = MiscParser.Maybev.t and type location = A.location
 = struct
...
(******************************************************)
  ...
  let finish_atom f_reg a =
    ...
    match a with
    | LV (loc,v) -> LV (finish_location f_reg loc, v)
...
(**********************************)
  ...
  let collect_state_atom (loc,(_:Maybev.t)) = collect_location loc
