...
/*
  Having a fast (unlocked) rand function is critical for speed.
  Quality of generator is less important.
*/
...
static const unsigned a = 69069;
static const unsigned c = 25173 ;
// static unsigned st = 0;

inline static unsigned unlocked_rand(st_t *st)  {
  unsigned r = a * *st + c ;
  *st = r ;
  /* Swap high & low bits */
  unsigned int low = r & 0xffff ;
  unsigned int high = r >> 16 ;
  r = high | (low << 16) ;
  return r ;
}

int rand_bit(st_t *st)  {
  unsigned r = unlocked_rand(st) ;
  r &= 1 ;
  return r ; 
}

static const unsigned r_max = UINT_MAX ;

unsigned rand_k (unsigned *st,unsigned k) {
  unsigned r, v ;
  do {
    r = unlocked_rand(st) ;
    v = r % k ;
