...
module type Config = Language.Config

module Make(Cfg:Language.Config) (T:Test.S) (O:Indent.S) (Lang : Language.S) : sig
...
(* Re-initialization, called once per run *)
  ...
  module Lang = Lang(Cfg)(O)(struct
      module T = T

      module LocSet = LocSet

      let do_affinity = do_affinity
      let do_timebase = do_timebase
      let have_timebase = have_timebase
      let do_custom = do_custom
      let do_verbose_barrier_local = do_verbose_barrier_local
      let do_collect_local = do_collect_local
      let do_safer = do_safer
      let do_verbose_barrier = do_verbose_barrier
      let do_force_affinity = do_force_affinity
      let do_check_globals = do_check_globals
      let barrier = barrier
      let do_staticpl = do_staticpl
      let stride = stride
      let find_global_type = find_global_type
      let dump_global_type = dump_global_type
      let dump_type = dump_type
      let indent3 = indent3
      let loop_test_prelude = loop_test_prelude
      let indent = indent
      let indent2 = indent2
      let dump_a_addr = dump_a_addr
      let get_global_names = get_global_names
      let get_prefetch_info = get_prefetch_info
      let do_staticNpl = do_staticNpl
      let do_isync = do_isync
      let get_final_locs = get_final_locs
      let dump_cond_fun_call = dump_cond_fun_call
      let dump_ctx_loc = dump_ctx_loc
      let test_witness = test_witness
      let dump_loc_name = dump_loc_name
      let find_type = find_type
      let remark_pos = remark_pos
      let loop_test_postlude = loop_test_postlude
      let do_collect_after = do_collect_after
      let have_finals_globals = have_finals_globals
    end)

  let dump_templates = Lang.dump_test
