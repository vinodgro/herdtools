...
(* Typing *)
      let type_event env e =
        ...
        | Atomic -> A.Atomic A.deftype
        | Plain -> A.Plain A.deftype
        | Reserve -> Warn.user_error "No reservation in C" in
        try
          let ty0 = StringMap.find loc env in
          if ty0 <> ty then
            Warn.user_error "Type mismatch on location %s" loc ;
          env
        with
...
(* Basics *)
      ...
      let compile_store e = 
        ...
        let loc = e.C.loc in
        match e.C.atom with
        | Reserve -> Warn.user_error "No reservation in C"
        | Plain ->
            A.Store (loc,A.Const v)
        | Atomic ->
            A.AtomicStore(A.SC,loc,A.Const v)
