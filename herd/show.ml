...
module Make(O:PrettyConf.S) = struct
  ...
  let do_show_file name_dot =    
    let name_ps = psfile name_dot in
    Handler.push (fun () -> my_remove name_ps) ;
    let cmd =
      Printf.sprintf
        "%s -Tps %s > %s 2>/dev/null && gv %s 2>/dev/null && /bin/rm -f %s"
        generator
      name_dot name_ps name_ps name_ps in
    ignore (Sys.command cmd) ;
    Handler.pop ()


let show_file name_dot = if O.gv then do_show_file name_dot
