...
module Make (A : Arch.S) : sig
 ...
 type action =    
    | Access of Dir.dirn * A.location * A.V.v * OpenCLBase.mem_order * OpenCLBase.mem_scope * bool (* is_failed_rmw *) * bool (* is_remote *)
    | Fence of OpenCLBase.gpu_memory_space list * OpenCLBase.mem_order * OpenCLBase.mem_scope * fence_type * bool (* is_remote *)
    | RMW of A.location * A.V.v * A.V.v * OpenCLBase.mem_order * OpenCLBase.mem_scope * bool (* is_remote *)
  ...
  type action = 
    | Access of dirn * A.location * V.v * OpenCLBase.mem_order * OpenCLBase.mem_scope * bool (* is_failed_rmw *) * bool (* is_remote *)
    | Fence of OpenCLBase.gpu_memory_space list * OpenCLBase.mem_order * OpenCLBase.mem_scope * fence_type * bool (* is_remote *)
    | RMW of A.location * V.v * V.v * OpenCLBase.mem_order * OpenCLBase.mem_scope * bool (* is_remote *)
    | Blocked_RMW of A.location
 
  let mk_init_write l v = Access (W,l,v,OpenCLBase.NA,OpenCLBase.S_workitem,false, false)
...
(* relative to memory *)
    ...
    | RMW (A.Location_global _,_,_,_,_,_)
        -> true
    | _ -> false
    ...
    | Access (_,A.Location_global _,_,_,_,_,_) -> true
    ...
    | Access (_,A.Location_global _,_,mo,_,_,_) -> mo != OpenCLBase.NA
    | RMW _ -> true
    | _ -> false
...
(* Barriers *)
    ...
    let same_barrier_id a1 a2 = 
      ...
      in
      match a1, a2 with
      | Fence (_,_,_,ft1,_), Fence (_,_,_,ft2,_) ->
...
(* Fences *)
   ...
   let is_global_fence a = match a with
     | Fence (rs,_,_,_,_) -> List.mem OpenCLBase.GlobalMem rs
     | _ -> false

   let is_local_fence a = match a with
     | Fence (rs,_,_,_,_) -> List.mem OpenCLBase.LocalMem rs
     | _ -> false

   let is_entry_fence a = match a with
     | Fence (_,_,_,Entry_fence _,_) -> true
     | _ -> false
   
   let is_exit_fence a = match a with
     | Fence (_,_,_,Exit_fence _,_) -> true
     | _ -> false
...
(* Mutex operations *)
   ...
   let mo_matches target a = match a with
     | Access(_,_,_,mo,_,_,_)
     | RMW (_,_,_,mo,_,_) 
     | Fence (_,mo,_,_,_) -> mo=target
     | _ -> false

   let is_sc_action a = mo_matches OpenCLBase.SC a

   let scope_matches target a = match a with
     | Access(_,_,_,_,s,_,_)
     | RMW (_,_,_,_,s,_) 
     | Fence (_,_,s,_,_) -> s=target
     | _ -> false
       
   let is_failed_rmw a = match a with
     | Access(_,_,_,_,_,b,_) -> b
     | _ -> false

   let is_remote a = match a with
     | Access(_,_,_,_,_,_,rem) -> rem
     | RMW (_,_,_,_,_,rem) -> rem
     | Fence (_,_,_,_,rem) -> rem
     | _ -> false
